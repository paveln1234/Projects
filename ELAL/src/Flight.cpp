...
bool Flight::SetSeat(const string& _seatNumber)
{
    for(map<string, Tier_t*>::const_iterator it = m_tiers.begin(); it != m_tiers.end(); it++ )
    {
        Tier_t* tempTier = it->second;
        if(tempTier->SetSeat(_seatNumber))
        {
            ++m_Passengers;
            return true;
        }
...
bool Flight::RealesSeat(const string& _seatNumber)
{
    for(map<string, Tier_t*>::const_iterator it = m_tiers.begin(); it != m_tiers.end(); it++ )
    {
        Tier_t* tempTier = it->second;
        if(tempTier->RealesSeat(_seatNumber))
        {
            --m_Passengers;
            return true;
        }
...
size_t Flight::GetTierPrise(UserFlightInterface::FlightTier _tier)const
{
    Tier_t* temptier = GetTair(_tier);
    return temptier->GetPrise();
}
