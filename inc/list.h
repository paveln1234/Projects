...
typedef enum List_Result {
	LIST_UNINITIALIZED_ERROR = -1,			/**< Uninitialized list 					 	*/
	LIST_SUCCESS,
 ...
 * @warning no error will return if destroy failed.
 */
 ...
 * @params _item An item to add to the list head
 * @returns error code
 ...
 *
 * @params _list A previously created List ADT returned via ListCreate
 * @params _item An item to add to the list tail
 * @returns error code
 * @retval LIST_SUCCESS on success
 * @retval LIST_UNINITIALIZED_ERROR if the list is not initialized
 * @retval LIST_NULL_ELEMENT_ERROR  if _item is NULL
 ...
 * @warning if the list point to NULL or ilegal pointer given then will return 0.
 */
